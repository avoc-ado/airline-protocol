...
- Seed script that initializes market, reserves, and writes IDL metadata used by the UI
...
- Seed script should:
  ...
  - Create Pyth push feed accounts and set deterministic prices (preloaded via genesis fixtures)
...
## IDL metadata bundle
Canonical path: `apps/web/public/idl/airline.json` (`AIRLINE_IDL_PATH` overrides).

Schema (high level):
- `idl`: Anchor IDL JSON (object)
- `metadata`:
  - `version`: `"v1"`
  - `cluster`: `"localnet" | "devnet" | "mainnet-beta" | ...`
  - `generatedAt`: ISO timestamp
  - `programId`: on-chain program id
  - `marketAddress` (optional)
  - `reserves`: array of `{ address, mintAddress, name, oracleAddress }`
  - `openbookProgramId` (optional)
  - `pythPushProgramId` (optional)
  - `pythFeeds` (optional): `{ label, account, feedId }[]`
  - `seedStatus`: `"seeded" | "stub"`

Consumers:
- Web loads `/idl/airline.json` in-browser.
- CLI/tests load via `AIRLINE_IDL_PATH`.
...
- Loads IDL metadata generated by seed
...
- `yarn dev:all` -> start localnet + seed + start UI + watch shared packages
...
Port allocator: `scripts/lib/ports.mjs`.
- Default: allocate ephemeral ports per run.
- Deterministic block: set `AIRLINE_TEST_PORT_BASE`, optionally `AIRLINE_AGENT_ID`.
  - rpc = base + (agentId * 20) + 0
  - ws = base + (agentId * 20) + 1
  - web = base + (agentId * 20) + 2
  - testPortBase = base + (agentId * 20) + 10
- Per-port overrides: `AIRLINE_RPC_PORT`, `AIRLINE_WS_PORT`, `AIRLINE_WEB_PORT`.

Standard env vars (non-exhaustive):
- Ports: `AIRLINE_RPC_PORT`, `AIRLINE_WS_PORT`, `AIRLINE_WEB_PORT`, `AIRLINE_TEST_PORT_BASE`
- URLs: `AIRLINE_RPC_URL`, `AIRLINE_WS_URL`
- IDs/paths: `AIRLINE_PROGRAM_ID`, `AIRLINE_MARKET_ADDRESS`, `AIRLINE_IDL_PATH`, `AIRLINE_OPENBOOK_V2_PROGRAM_ID`, `AIRLINE_PYTH_PUSH_PROGRAM_ID`, `AIRLINE_PYTH_ACCOUNT_DIR`
- Cluster: `AIRLINE_CLUSTER`
- Validator: `AIRLINE_TEST_VALIDATOR_BIN`, `AIRLINE_LEDGER_DIR`, `AIRLINE_LEDGER_BASE_DIR`, `AIRLINE_KEEP_LEDGER`, `AIRLINE_TMP_DIR`, `AIRLINE_TEST_VALIDATOR_QUIET`
- Agent isolation: `AIRLINE_AGENT_ID`, `AIRLINE_PROFILE_DIR`
